...
function product_parsing($url_item, $number_product, $category_list, $category_breadcrumbs_last) {
  ...
  $table_row_char_item = [];
  for ($i=0; $i < count($characteristics_data_value_all); $i++) { 
    $table_row_char_item[$i] = '<tr><td class="label">'.$characteristics_data_label[$i].'</td>'.'<td class="value">'.$characteristics_data_value[$i].'</tr>';
  }

  $table_row_char = '';
  for ($i=0; $i < count($characteristics_data_value_all); $i++) { 
    $table_row_char .= $table_row_char_item[$i];
  }

  $characteristics = '<table class="characteristics_block"><tbody>' . $table_row_char . '</tbody></table>';
  ...
  // $size_table_3 = preg_replace('/ /', '', $size_table_3);
  // $size_table_3 = pq($size_table_3);
  // $size_table_3 = $size_table_3->text();


  // $size_arr = explode('|', $size_table_3);
  // for ($i=0; $i < count($size_arr); $i++) { // вивести по масиву значень Size
