...
function category_url_product($url_category) {
  ...
  foreach ($doc_category->find('.product-name a') as $item) {
    $item = pq($item);
    $url_product = $item->attr('href');
    $url_product = 'http://joma-style.com.ua/' . $url_product;
...
function product_parsing($url_item, $number_product, $category_list, $category_breadcrumbs_last) {
  ...
  $url = $url_item;
  // sku
  $sku = $doc->find('.f-left .sku')->text();
  $sku = preg_replace('/Артикул: /', '', $sku);

  // desc_full
  $desc_full = $doc->find('.short-description .std')->html();
  ...
  $main_image_url = $doc->find('.cloud-zoom')->attr('href');
  $main_image_url = 'http://joma-style.com.ua' . $main_image_url;
  ...
  // for ($i=0; $i < count($size_foot); $i++) { // вивести по масиву значень Size
    ...
    echo '<td class="category_list">Аксесуари > М\'ячі > FUTSAL</td>';
