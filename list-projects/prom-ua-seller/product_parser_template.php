...
function product_parsing($url_product, $desc, $product_details, $hosting_url)
{
	...
	//Format XML
	echo '<offer id="' . $offer_id . '" available="true">';
	...
	//images
	for ($i = 0; $i < count($all_images_name_arr); $i++) {
		echo '<picture>' . $hosting_url . 'images_products/' . $offer_id . '/' . $all_images_name_arr[$i] . '</picture>';
	}
	echo '<name>' . $name . '</name>';
	...
	for ($i = 0; $i < count($characteristics_name); $i++) {
		echo '<param name="' . $characteristics_name[$i] . '">' . $characteristics_value[$i] . '</param>';
	}
